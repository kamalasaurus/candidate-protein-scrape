export default async function scraper() {
  ...
  for (const gene of Object.keys(gene_to_protein_list)) {
    for (const protein_list of gene_to_protein[gene]) {
      let proteins = []
      for (const protein of protein_list.split(',')) {
        try {
          ...
          proteins.push({gene, protein, sequence, length, score})
        } catch (error) {
          console.error('Error fetching data:', error);
        }
      }
      const exemplar = proteins.sort((a, b) => b.score - a.score)[0]
      const {gene, protein, sequence, length, score} = exemplar
      const header = `>${gene} ${protein} ${length} ${score}`
      await logger.write(encoder.encode(`${header}\n${sequence}\n`))
      console.log(header, sequence)

      multiple_exemplars = proteins.filter(p => p.score === exemplar.score)

      if (multiple_exemplars.length > 1) {
        const row = `${gene}\t${exemplar.score}\t${multiple_examplars.map(p => p.protein).join(',')}\t${multiple_examplars.map(p => p.sequence).join(',')}`
